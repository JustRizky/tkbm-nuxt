    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div class="lg:col-span-2 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="h-64 flex items-end justify-between gap-2 pt-4 border-l border-b border-gray-100 relative">
          <div class="absolute inset-0 flex flex-col justify-between pointer-events-none pr-2">
            <div v-for="n in 6" :key="n" class="border-t border-gray-50 w-full text-[10px] text-gray-300 flex justify-end items-start">{{ 11 - n }}</div>
          </div>
          
          <div v-for="(val, day) in {Senin: 80, Selasa: 100, Rabu: 100, Kamis: 75, Jumat: 80, Sabtu: 65, Minggu: 65}" :key="day" class="relative group flex-1 flex flex-col items-center">
            <div :style="{height: val + '%'}" class="w-full max-w-[40px] bg-blue-900 rounded-t-sm transition-all hover:bg-blue-800"></div>
      ...
      <div class="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        ...
        <div class="relative w-48 h-48 mx-auto mb-8">
          <div class="absolute inset-0 rounded-full border-[25px] border-blue-600" style="clip-path: polygon(50% 50%, 100% 0, 100% 50%);"></div>
          <div class="absolute inset-0 rounded-full border-[25px] border-orange-400" style="clip-path: polygon(50% 50%, 0 0, 100% 0);"></div>
          <div class="absolute inset-0 rounded-full border-[25px] border-rose-600" style="clip-path: polygon(50% 50%, 0 0, 0 100%, 100% 100%, 100% 50%);"></div>
        </div>

        <div class="space-y-2">
          <div v-for="(color, shift) in {'bg-blue-600': 'Shift I', 'bg-rose-600': 'Shift II', 'bg-orange-400': 'Shift III'}" :key="shift" class="flex items-center gap-2">
        ...
        <div class="flex gap-4">
          <div class="flex-1 p-6 bg-slate-50 rounded-xl border border-gray-100">
            ...
            <NuxtLink to="#" class="text-blue-600 text-[10px] font-bold flex items-center gap-1 uppercase">
              Baca Artikel <ArrowUpRight class="w-3 h-3" />
            </NuxtLink>
          </div>
          </div>
      </div>
